...
class StatsDashboard(tk.Tk):
    ...
    def _generate_pdf_report(self, path):
        ...
        # ── title ────────────────────────────────────────────────────────────
        ax_title = fig.add_axes([0.06, 0.87, 0.88, 0.05])
        ax_title.axis("off")
        ...
        # ── summary table ────────────────────────────────────────────────────
        ax_table = fig.add_axes([0.06, 0.51, 0.88, 0.34])
        ax_table.axis("off")
        ...
        # ── histogram ────────────────────────────────────────────────────────
        ax_hist = fig.add_axes([0.08, 0.26, 0.38, 0.18])
        ...
        # ── box plot ─────────────────────────────────────────────────────────
        ax_box = fig.add_axes([0.56, 0.26, 0.38, 0.18])
        ax_box.boxplot(self.data, orientation="vertical", patch_artist=True,
                       ...
                       flierprops=dict(marker="o", markerfacecolor="#e74c3c",
                                       markersize=5, alpha=0.6))
        ax_box.set_title("Box Plot of Grades", fontsize=10, fontweight="bold")
        ax_box.set_ylabel("Grade")
        ax_box.set_xticks([])

        for label, val in [("Q1", s["Q1"]), ("Med", s["Median"]), ("Q3", s["Q3"])]:
            ax_box.text(0.65, val, f"{label}={val:.1f}",
                       fontsize=7, va="center", color="#333")

        # ── interpretation ───────────────────────────────────────────────────
        ax_interp = fig.add_axes([0.06, 0.04, 0.88, 0.14])
        ax_interp.axis("off")
